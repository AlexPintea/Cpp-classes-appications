...
// checks if there are any common entries in string[]-s
bool simmilar_items ( string a[], string b[] )
{
	int min_size;
...
// Simmilar
void simmilar( int iter )
{
	fstream f;

	// get entry file filenames
	f.open( "entry_files" );
	int files_iter;
	string file_strings[1000];
	f >> files_iter;
	...
	// get entries of this file as strings and saves entries[ iter ] as string[]
	f.open( name + "_entries" );
	int this_iter; // for items of this file
	f >> this_iter;
	string this_entry = "";
	string this_entries[1000];

	for ( int i=0; i<this_iter; i=i+1 )
	{
		getline( f, this_entries[i]);
		if ( i == iter )
		{
			this_entry = this_entries[i];
			break;
		}
	}
	this_entries = {};
	move_words( this_entry, this_entries )	f.close();


	// get entries of simmilar files and compare
	bool have_simmilar = false;	for ( int i=0; i<columns_iter; i=i+1 )
	{
		if ( files_1[i] == name + "_entries" )
		{
			f.open( files_2[i] );
			int f_iter;
			f >> f_iter;
			string entry = ;
			string entries[1000];
			for ( int l=0; l<f_iter; l=l+1 )
			{
				getline( f, entry );
				move_words( entry, entries );
				if ( simmilar_items( entries, this_entries ) )
				{
					if ( have_simmilar == false )
					{
						have_simmilar = true;
						cout << "\nSimmilars:\n";
					}
