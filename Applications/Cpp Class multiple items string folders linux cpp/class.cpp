...
{
	// delete file
	string command_delete = "rm - R " + filename;
...
// enter a linux command as a string
void command ( string command_string )
{
...
// check if a file was not already named so
bool is_filename_taken( string filename )
{
	for ( int i=0; i<files_iter; i=i+1 )
		if ( entry_files_strings[i] == filename )
			return true;
...
// save  entry_files_strings[]  into   entry_files  including a filename
...
void save_entry_files() 
{
	...
	for ( int i=0; i<files_iter; i=i+1 )
		if ( entry_files_strings[i] == "9174_delete" )
			have_deleted = true;
...
void edit_class_file()
{
	...
	if ( files_iter == 0 )
	{
		cout << "Entry files were not made yet.\n";
		string choice;
		cout << "If you want to make an entry file, type \'y\' ( y / n ): ";
		cin >> choice;
		if ( choice == "y" )
			make_class_file();
		else
			cout << "Exiting to \'class\'.\n";
		return;
	}
	...
	{
		cout << "Invalid num. of file. Retry.\n";
		return;
	}
	...
	string command_file = "./" + entry_file + '/' + entry_file;
	command( command_file );

}

// delete a class file and its entries
void delete_class_file()
{
	...
	int choice;
	cout << "Enter num. of file you want to delete: ";
	cin >> choice;
	...
	string entry_file = entry_files_strings[ choice ];

	cout << "Sure to completly delete files: \'" << entry_file << "\' and \'" << entry_file << "_entries\' along with its simmilarities ( y / n ): ";
	string sure;
	cin >> sure;
	...
	if ( sure == "y" )
	{
		string command_delete = "rm -R " + entry_file;
		command( command_delete );
		cout << "Files: \'" << entry_file << "\' and \'" << entry_file << "_entries\' were deleted.\n";



		entry_files_strings[ choice ] = "9174_delete";
		...
		for ( int i=0; i<columns_iter; i=i+1 )
		{
			...
			if ( files_1[i] == entry_file + "_entries" or files_2[i] == entry_file + "_entries" )
				count = count + 1;
		}
		f.close();
		...
		cout << "Aborted deleting files: \'" << entry_file << "\' and \'" << entry_file << "_entries\'\n";
}
...
{
	string filename = entry_files_strings[iter] + '/' + entry_files_strings[iter] + "_entries";
...
string get_simmilar_choice ( int iter )
{
	...
	string filename = entry_files_strings[ iter ] + '/' + entry_files_strings[ iter ] + "_entries";
	string name_of_file = entry_files_strings[ iter ] + "_entries";
	...
	for ( int i=0; i<files_iter; i=i+1 )
	{
		...
		for ( int l=0; l<size; l=l+1 )
			...
			{
				string current_filename = entry_files_strings[i] + '/' + entry_files_strings[i] + "_entries";
				string name_of_current_file = entry_files_strings[i] + "_entries";

				if ( types[k] == file_types[l] and !is_simmilar_made( name_of_file, file_parameters[l], name_of_current_file, parameters[k] ) )
				{
					cout << counter + 1 << ". \"";
					cout << name_of_file << "\": \'" << file_parameters[l] << "\'   / and /   \"";
					cout << name_of_current_file << "\": \'";
					cout << parameters[k] << "\' - both are type \'" << types[k] << "\'\n"; 

					string current_choice = name_of_file + " " + file_parameters[l] + "   " + name_of_current_file + " " + parameters[k];
...
void edit_simmilar_file()
{
	...
	cout << "Choices:  1. add simmilar files   2. remove simmilar files   3. show simmilar files\n\n";
	cout << "Your choice: ";
	cin >> choice_string;
	cout << "\n";
	...
	if ( choice_string == "1" )
	{
		...
		cout << choice_available_simmilar << "\n";
...
bool have_main ( string a )
{
	string strings[ 10000 ];
	int strings_iter;

	move_words( a, strings );
...
void get_data_1( string &name, string parameters[], string types[], int &num_parameters )
{
	...
	if ( is_filename_taken( name ) == false )
	{
		filename = name + ".cpp";
...
void make_class_file()
{
	...
	string empty; // getline has to be done to continue
	getline( cin, empty );
	...
	file << "{\n";
	file << "	ifstream f;\n";
	file << "	f.open( filename + \'/\' + filename + \"_entries\" );\n";
	file << "	int num;\n";
	file << "	string entry;\n";
	...
	file << "		ifstream f;\n";
	file << "		f.open( name + \'/\' + name + \"_entries\" );\n";
...
//	file << "\n   /     \n";
	...
	file << "			\n";
	file << "			ifstream fi;\n";
	file << "			fi.open( files[i] + \'/\' + files[i] + \"_entries\" );\n";
	...
	file << "								remaining_files[ counter ] = files[i];\n";
	file << "								counter = counter + 1;\n";
	file << "							}\n";
	file << "						}\n";
...
//	file << "		}\n";
	...
	ofstream item_file;
	string item_file_name = name + '/' + name + "_entries";
	...
	// compile the file and delete the cpp
	string command_compile = "g++ -o " + name + '/' + name + " " + name + '/' + name + ".cpp";
	...
	while ( getline( fi, temp ) )
	{
		...
		if ( have_main( temp ) ) break;

		file_cpp = file_cpp + temp + '\n';
	}
...
int main ()
{
	...
	if ( entry_files_strings[0] == "" ) // if  entry_files  was not made
	{
		ofstream entry_files_ofstream;
		entry_files_ofstream.open( "entry_files" );
		entry_files_ofstream << 0;
		entry_files_ofstream.close();
	}

	string choice = "";

	cout << "Operations:  make. Make class file         edit. Edit / Show a class file    simmilar. Enter simmilar files\n";
	cout << "             delete. Delete a class file   exit. Exit\n\n";
	...
	// for invalid data
	if ( choice != "make" and choice != "edit" and choice !="exit" and choice != "delete" and choice != "simmilar" )
	{
